...
async function captureStart(projectDir, options) {
  ...
  const commandSpec = commandForPlatform("npm", ["run", "start"]);
  const child = spawn(commandSpec.command, commandSpec.args, {
    cwd: projectDir,
  ...
  const outputPath = join(projectDir, ".nest-capture-raw.log");
  const rawOutput = createWriteStream(outputPath, { flags: "a" });
  ...
  return {
    entries,
    cleanLines: entries
      .filter((entry) => typeof entry.line === "string" && stripAnsi(entry.line).includes("[Nest]"))
...
The command creates a fresh official Nest CLI template, optionally applies the ${mode} observation variant, starts it with \`PORT=0 npm run start\`, and records every stdout/stderr line with \`elapsedMs\`.
